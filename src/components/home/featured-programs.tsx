      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <FadeIn className="mb-12">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <span className="section-label mb-4">{tSec("ourOfferings")}</span>
              ...
              <StaggerItem key={p.id}>
                <article className="glass-card group flex h-full flex-col overflow-hidden">
                  ...
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    {/* Category badge */}
                    <span
                      className="absolute start-3 top-3 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider backdrop-blur-sm"
                      style={{ borderColor: `${accentColor}60`, color: accentColor, background: "rgba(12,44,89,0.85)" }}
                    >
                      {tProg(`category${p.category.charAt(0).toUpperCase() + p.category.slice(1)}` as "categoryManagement")}
                    </span>
                  </div>
